...
class TestSimpleScenarios(unittest.TestCase):
    def testDefaultInit(self):
        with tempfile.TemporaryDirectory() as toplevel_dir:
            logger = l2logger.DataLogger(toplevel_dir, 'test_scenario')
            self.assertTrue(os.path.exists(logger.scenario_dir))
            self.assertEqual(json.dumps(logger.column_info), json.dumps({'metrics_columns': []}))
            self.assertEqual(json.dumps(logger.scenario_info), json.dumps({}))

    def testErrorInit(self):
        with tempfile.TemporaryDirectory() as toplevel_dir:
            self.helperErrorInit(toplevel_dir, 'test_scenario', True)
            self.helperErrorInit(toplevel_dir, 'test_scenario', {'metrics_columns': {}})
            self.helperErrorInit(toplevel_dir, 'test_scenario', {'metrics_columns': ['test', 1]})
            self.helperErrorInit(toplevel_dir, 'test_scenario', ['metrics_columns'])
            self.helperErrorInit(toplevel_dir, 'test_scenario', {'col1': {}})
    
    def helperErrorInit(self, top_dir, scenario, column_info=None, scenario_info=None):
        self.assertRaises(Exception, l2logger.DataLogger,
            top_dir, scenario, column_info, scenario_info)
    ...
    def testValidRecord(self):
        with tempfile.TemporaryDirectory() as toplevel_dir:
            ...
            validHelper = lambda records: self.helperValidRecord(toplevel_dir, metric_cols, records)
    ...
    def testErrorRecord(self):
        with tempfile.TemporaryDirectory() as toplevel_dir:
            ...
            errHelper = lambda records: self.helperErrorRecord(toplevel_dir, metric_cols, records)
            # missing cols
